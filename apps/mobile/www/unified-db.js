(function(){
  ...
  function eventFor(type,id,operation,payload,baseRevision){return{id:uuid(),entityType:type,entityId:id,operation,payload,deviceId:deviceId(),userName:user(),createdAt:now(),baseRevision:baseRevision||0,appVersion:'1.9.43'}}
  ...
  function softDelete(type,id,metadata={}){let d=read(),old=d.entities[type]?.[id];if(!old)return false;let timestamp=now(),record={...old,...metadata,revision:(old.revision||0)+1,deletedAt:timestamp,updatedAt:timestamp,updatedBy:user(),deviceId:deviceId()};d.entities[type][id]=record;d.trash.push({type,id,deletedAt:timestamp});let ev=eventFor(type,id,'delete',record,old.revision||0);d.events.push(ev);d.queue.push(ev.id);write(d);return true}
  ...
  function hardDelete(type,id){let d=read();if(!d.entities[type]?.[id])return false;delete d.entities[type][id];d.trash=d.trash.filter(x=>!(x.type===type&&x.id===id));d.queue=d.queue.filter(eventId=>!d.events.some(e=>e.id===eventId&&e.entityType===type&&e.entityId===id));d.events=d.events.filter(e=>!(e.entityType===type&&e.entityId===id));write(d);return true}
